...
router.post('/login', (req, res, next) => {
  ...
  // 確認有沒有這個帳號
  let [members] = await pool.execute('SELECT id, email, password, name, photo FROM members WHERE email = ?', [req.body.email]);
  if(members.length === 0) {
    // 如果沒有，就回覆錯誤
    // 這個 email 沒有註冊過
    return res.status(400).json({ code: 3003, error: '帳號密碼錯誤' });
  }
  ...
  // TODO: 密碼符合，就開始寫 session(是用 session/cookie 的方式完成) -> 可以用 jwt 方式取代
  // TODO: 回覆資料給前端
  res.json({ code: 0 , member: { email: member.email, name: member.name, photo: member.photo } });
});
